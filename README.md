...
This AEM library automates invalidating cached content in CDNs when publishing updates in AEM. It improves website performance, security, and flexibility.
...
- **Flexible CDN integrations** - Supports Akamai out-of-the-box. Easily extend for other CDNs via APIs
...
- **Logs and monitoring** - Detailed logging for tracking and alerts integration
- **APIs and extension points** - Customize logic by overriding key hooks or implementing interfaces

## How to use 

This project supports Akamai CDN by default and this chapter will show you how to use the built-in support, but you can customize the invalidation behaviour by changing the configurations according to your needs.

If you want to use a different CDN provider or modify the default logic, you can also extend the code by overriding some hook methods. See [Implementing a Custom CDN Provider](#Implementing a Custom CDN Provider) for more details.

### Requirements

When installed the project will create a service user named autocdninvalidatorReadService (see [here](ui.config/src/main/content/jcr_root/apps/autocdninvalidator/osgiconfig/config/org.apache.sling.jcr.repoinit.RepositoryInitializer~autocdninvalidator.cfg.json)). 

It is mandatory to verify that service user have read access to all the paths configured to be listened for.

### Configuring and Using Akamai CDN Invalidation

Module ui.config.example provides out-of-the-box support for invalidating content in the Akamai CDN. Sample configurations are included that allow invalidating content by [URL](ui.config.example/src/main/content/jcr_root/apps/autocdninvalidator-example/osgiconfig/config/com.baglio.autocdninvalidator.core.jobs.ProductPagesInvalidationJobConsumer.cfg.json) or by [tag](ui.config.example/src/main/content/jcr_root/apps/autocdninvalidator-example/osgiconfig/config/com.baglio.autocdninvalidator.core.jobs.EditorialAssetInvalidationJobConsumer.cfg.json).

To enable Akamai CDN invalidation, you need to:


1. Install the sample configurations from the ui.config.example module, or create your own configurations under the /apps folder.
2. Update the placeholder values in the AkamaiInvalidationServiceImpl [configuration](ui.config.example/src/main/content/jcr_root/apps/autocdninvalidator-example/osgiconfig/config/com.baglio.autocdninvalidator.core.service.impl.AkamaiInvalidationServiceImpl.cfg.json) file with your credentials and settings from the Akamai portal: *hostname*, *getAkamaiClientToken*, *getAkamaiAccessToken*, *getAkamaiClientSecret*.

The included sample configurations demonstrate invalidating:

- Pages under the website root by tag
- Child pages under a specific root page by URL

![Akamai processing diagram](assets/Akamai-diagram.jpg)

The above diagram illustrates how different parts of the code work together to perform invalidation on the Akamai CDN. The diagram consists of three main components:
...
To use the library with other CDN providers:
...
You can also override methods on the AbstractInvalidationJob class if you need to preprocess data or customize the invalidation logic:
...
![Job processing diagram](assets/JobProcessing-diagram.jpg)
...
- [ReplicationEventListener](core/src/main/java/com/baglio/autocdninvalidator/core/listeners/ReplicationEventListener.java) and [DynamicResourceChangeListener](core/src/main/java/com/baglio/autocdninvalidator/core/listeners/DynamicResourceChangeListener.java): these services are OSGI factories and have the following configuration fields:
  - Enable: a boolean flag that indicates whether the service is enabled or not
  - Filter Paths: a string array that defines the root paths for observed events. Regex are not supported here.
  - Events: a string array that defines the array of change types, such as CHANGED, ADDED, or REMOVED
  - Target Job Topic: a string that defines which job consumer will be used to process data
  - Filter Regex: a string that defines the pattern to identify page or asset which must be processed
- [EditorialAssetInvalidationJobConsumer](core/src/main/java/com/baglio/autocdninvalidator/core/jobs/EditorialAssetInvalidationJobConsumer.java): this service is also an OSGI factory and has the following configuration fields:
  - Enable: a boolean flag that indicates whether the service is enabled or not
  - Job Topic: a string array that defines which topic this consumer is able to process
  - CDN Configuration ID: a string that defines which CDN configuration to leverage
  - Type of Invalidation: a string that defines the type of invalidation to be leveraged, either URLs, tag, or code
  - Tag/Code Mappings: a string array that defines the pattern to tag or code associations for invalidation rules.
...
This listens for changes under /content/we-retail and /content/wkn. It matches pages and assets, while filtering out paths containing "products" or "adventures".
...
For testing locally without an actual CDN, you can simulate CDN calls using [mendhak/http-https-echo](https://github.com/mendhak/docker-http-https-echo).
...
# Planned Enhancements
 ...
 - Junit testing: Add/Improve unit and integration test coverage using JUnit
