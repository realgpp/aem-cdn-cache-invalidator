...
class AkamaiInvalidationServiceImplTest {
  ...
  void testInvalidationFailure() throws IOException {
    ...
    invalidationService.activate(config);
    boolean result = invalidationService.purgeByTag(null);
    assertFalse(result);

    // test null input
    when(config.isEnabled()).thenReturn(true);
    invalidationService.activate(config);
    result = invalidationService.purgeByTag(null);
    assertFalse(result);

    // test empty input
    when(config.isEnabled()).thenReturn(true);
    invalidationService.activate(config);
    result = invalidationService.purgeByTag(new HashSet<>());
    assertFalse(result);

    // test client is null
    when(config.isEnabled()).thenReturn(true);
    invalidationService.activate(config);
    result = invalidationService.purgeByTag(tags);
    assertFalse(result);
    ...
    invalidationService.activate(config);
    result = invalidationService.purgeByURLs(tags);
    assertFalse(result);
  }
  ...
  void testInvalidation() throws IOException {
    ...
    boolean result = invalidationService.purgeByTag(tags);
    assertTrue(result);

    // test invalidateByCode full execution with positive result
    result = invalidationService.purgeByCode(tags);
    assertTrue(result);
